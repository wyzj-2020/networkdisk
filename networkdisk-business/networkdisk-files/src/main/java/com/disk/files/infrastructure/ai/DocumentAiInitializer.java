...
public class DocumentAiInitializer {
    ...
    public void scheduleInitialize(Long userId, Long userFileId, String filename) {
        if (userId == null || userFileId == null) {
            log.warn("scheduleInitialize skipped: userId or userFileId is null, userId={}, userFileId={}", userId, userFileId);
            return;
        }
        if (!supports(filename)) {
            log.warn("scheduleInitialize skipped: unsupported file type, filename={}", filename);
            return;
        }
        log.info("scheduleInitialize triggered: userId={}, userFileId={}, filename={}", userId, userFileId, filename);

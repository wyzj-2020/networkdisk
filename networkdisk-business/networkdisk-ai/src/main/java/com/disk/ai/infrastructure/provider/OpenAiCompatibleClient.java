...
public class OpenAiCompatibleClient {
        ...
        try {
            ChatCompletionResponse response = restClient.post()
                    ...
                    .onStatus(status -> status.value() >= 400, (req, res) -> {
                        ...
                        throw new AiException("AI provider returned " + res.getStatusCode() + ": " + errorBody, AiErrorCode.MODEL_REQUEST_FAILED);
                    })
            ...
            throw new AiException("Failed to request chat completion from the configured AI provider: " + e.getMessage(), e, AiErrorCode.MODEL_REQUEST_FAILED);
        }
    }
    ...
    public List<float[]> createEmbeddings(List<String> texts) {
        ...
        for (int i = 0; i < texts.size(); i += EMBEDDING_BATCH_SIZE) {
            ...
            List<float[]> batchResults = doCreateEmbeddings(batch);
            allEmbeddings.addAll(batchResults);
        }
        return allEmbeddings;
    }

    private List<float[]> doCreateEmbeddings(List<String> texts) {
        ...
        try {
            ...
            throw new AiException("Failed to request embeddings from the configured AI provider: " + e.getMessage(), e, AiErrorCode.MODEL_REQUEST_FAILED);
        }
    }
